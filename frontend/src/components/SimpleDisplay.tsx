...
function SimpleDisplay() {
  ...
  return (
    <div className="flex-1 min-h-0 rounded-md overflow-auto">
      <table className="border-spacing-2 border-separate relative">
        <thead className="text-sm">
          <tr>
            ...
            {services.map((service) => (
              <th
                className="font-bold p-2 shadow-inner bg-slate-950/30 border-slate-950 border-opacity-20 border-2 rounded-sm text-nowrap sticky"
                key={service.name}
              >
                {service.name}
              </th>
