...
async fn worker(idx: usize, rx: Receiver<Job<'_>>, exploit_name: &str, exploit_command: String, exploit_args: Option<String>) -> Result<()> {
    loop {
        ...
        match execute(job.request.payload(), exploit_name, &exploit_command, exploit_args.as_deref()).await {
            Err(err) => {
...
async fn execute(request: &ExecutionRequest, exploit_name: &str, exploit_command: &str, exploit_args: Option<&str>) -> Result<()> {
...
pub async fn main(runtime: AppRuntime, config: Config) -> Result<()> {
    ...
    let exploit_name = Box::leak(Box::new(config.runner_exploit.context("runner: the runner-exploit option is undefined")?));
    let exploit_command = config.runner_exploit_command.context("runner: the runner-exploit-command option is undefined")?;
    ...
    let mut stream = Box::leak(messaging).subscribe_execution_requests(exploit_name).await
    ...
    for i in 0..worker_count {
        ...
        spawn(worker(i, r, exploit_name, exploit_command.clone(), config.runner_exploit_args.clone()));
    }
